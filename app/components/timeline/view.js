          ...
          <Timeline>
            {
              tlItems.map((item) => (
                item.type === 'time'
                ?
                  <Item key={item.key} color="blue" dot={<Icon type="clock-circle-o" style={{fontSize: '16px'}} /> }>
                    <p className="timeline-item wow zoomIn animated">{item.key}</p>
                  </Item>
                :
        ...
        </div>
        {
          ...
          ?
            <div className="pagination" style={{marginTop: 0}}>
              <Pagination defaultCurrent={1} pageSize={count} current={current} total={total} onChange={this.onPageChange} />
            </div>
          :
          null
        }
